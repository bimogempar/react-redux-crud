  ...
  return (
    <div>
      <div className="flex justify-center">
        <button
          className="p-2 bg-blue-200 my-4"
      ...
      <div className="flex space-x-6 justify-center">
        <div>
          <div className="flex justify-center text-lg mb-5 p-2 bg-red-200">
            Belum Selesai
          </div>
          {loading ? (
            <div>Loading...</div>
            ...
            todos
              ...
              .map((todo) => (
                <div
                  key={todo.id}
                  className="flex justify-between space-x-24 border-2 border-gray-200 my-2 p-2"
                >
                  <div>{todo.title}</div>
                  <div className="space-x-4">
                    <input
                      type="checkbox"
                      onChange={() => {
                        handleUpdateStatusTodo(todo);
                      }}
                    />
                    <button onClick={() => handleClickEdit(todo)}>
                      <AiOutlineEdit />
                    </button>
                  </div>
                </div>
        ...
        <div>
          <div className="flex justify-center text-lg mb-5 p-2 bg-green-200">
            Selesai
          </div>
          {loading ? (
            <div>Loading...</div>
